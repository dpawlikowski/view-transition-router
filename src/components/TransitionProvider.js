...
export const TransitionProvider = ({ children, config = {} }) => {
    const [isPending, startTransition] = useTransition();
    const [direction, setDirection] = useState('replace');
    ...
    const _setTransition = useCallback((dir, type) => {
        setDirection(dir);
    ...
    useEffect(() => {
        const handlePopState = () => {
            ...
            const dir = prev !== undefined && next !== undefined
    ...
    return (_jsx(TransitionContext.Provider, { value: {
            direction,
            transitionType,
            isPending,
            config,
