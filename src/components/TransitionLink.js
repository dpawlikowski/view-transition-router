...
export const TransitionLink = ({ to, transition, duration: durationProp, easing: easingProp, replace: shouldReplace = false, target, navigate: navigateProp, renderLink: renderLinkProp, children, className, style, }) => {
    ...
    const doNavigate = () => {
        const opts = { replace: shouldReplace };
        if (navigate) {
            navigate(to, opts);
        }
    ...
    const handleClick = (e) => {
        ...
        const direction = shouldReplace ? 'replace' : 'forward';
    ...
    return (_jsx("a", { href: to, target: target, onClick: handleClick, className: className, style: style, children: children }));
};
